...
static cycle_t ixgbe_ptp_read_phc(struct ixgbe_hw *hw) {
	struct timespec64 ts;
	ts.tv_nsec = IXGBE_READ_REG(hw, IXGBE_SYSTIML);
	ts.tv_sec = IXGBE_READ_REG(hw, IXGBE_SYSTIMH);
...
void ixgbe_ptp_start_cyclecounter(struct ixgbe_adapter *adapter)
{
  ...
  strcpy(adapter->ops.name, "ixgbe1");
  /* adapter->ops.read = cc.read; */
  /* adapter->ops.arg = &adapter->hw_cc; */
  adapter->ops.read = ixgbe_ptp_read_phc;
  adapter->ops.arg = &adapter->hw;
  register_phc(&adapter->ops);
}
