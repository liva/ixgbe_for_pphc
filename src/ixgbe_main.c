...
#endif /* HAVE_INT_NDO_VLAN_RX_ADD_VID */
{
	...
	if (hw->mac.ops.set_vfta) {
		...
		hw->mac.ops.set_vfta(hw, vid, pool_ndx, true, true);
#else
		hw->mac.ops.set_vfta(hw, vid, pool_ndx, true, false);
#endif
		if (adapter->flags & IXGBE_FLAG_VMDQ_ENABLED) {
			int i;
			switch (adapter->hw.mac.type) {
 ...
 **/
static int ixgbe_probe2(struct pci_dev *pdev,
...
static int __devinit ixgbe_probe(struct pci_dev *pdev,
                                  const struct pci_device_id __always_unused *ent) {
  int i = ixgbe_probe2(pdev, ent);
  printk(KERN_INFO"probe return: %d\n", i);
  return i;
}
